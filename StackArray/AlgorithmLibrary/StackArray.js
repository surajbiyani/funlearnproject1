...
StackArray.prototype.setup = function()
{
	...
	// Changed the labeling of top from 0 to -1
...
//making the changes here
	...
	else if (this.top < SIZE && this.pushField.value == "" )
	{
	...
	{
		window.alert("Trying to Push A Null Value in Full Stack");	
	}
}
...
StackArray.prototype.push = function(elemToPush)
{
	...
	// Changed the value here from this.cmd("SetText", this.topID, this.top) to this.cmd("SetText", this.topID, this.top-1)
	this.cmd("SetHighlight", this.topID, 1);
	this.cmd("Step");
	this.top = this.top + 1;
	this.cmd("SetText", this.topID, this.top-1)
...
StackArray.prototype.clearAll = function()
{
	...
	if(this.top == 0)
	{
		window.alert("Stack is already Empty!")
	}
	else
	{
		this.top = 0;
