...
QueueArray.prototype.setup = function () {
	...
	this.cmd("CreateRectangle", this.tailID, -1, ARRAY_ELEM_WIDTH, ARRAY_ELEM_HEIGHT, TAIL_POS_X, TAIL_POS_Y);
...
QueueArray.prototype.enqueueCallback = function (event) {
	...
	else if ((this.tail+1) % SIZE != this.head && this.enqueueField.value == "" )
	{
		window.alert("Enter value to Push the Element");	
	}
	else if ((this.tail+1) % SIZE != this.head && this.enqueueField.value != "" )
	{
		window.alert("Queue OverFlow!");	
	}
	else if ((this.tail+1) % SIZE  && this.enqueueField.value == "" )
	{
		window.alert("Trying to Push A Null Value in Full Queue");	
	}
}
...
QueueArray.prototype.enqueue = function (elemToEnqueue) {
	...
	// changed here
	this.cmd("SetText", this.tailID, this.tail-1)
	this.cmd("Step");
...
QueueArray.prototype.clearAll = function () {
	...
	if ((this.head == 0) && (this.tail == 0)) {
		window.alert("Queue  is Already empty");
	}
	else {
		this.head = 0;
		this.tail = 0;
		...
		//Changed here
		this.cmd("SetText", this.tailID, "-1");
		return this.commands;
	}
}
